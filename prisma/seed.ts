...
async function main() {
  ...
  if (!existing) {
    ...
    console.log(`✅ Superadmin created: superadmin@system.com / ${pass}`)
  } else {
  ...
  // Create a default tenant + admin (optional, skip if already exists)
  ...
  // Wait, upsert needs a unique field. Category doesn't have a unique field except ID.
  // Let's seed categories with findFirst:
  ...
  if (!adminExists) {
    ...
    console.log('✅ Store admin created: admin@store.com / store1234')
  } else {
    console.log('ℹ️  Store admin already exists — skipping.')
  }

  console.log('\n🎉 Seed complete!\n')
  console.log('Login credentials:')
  console.log(`  Super Admin : superadmin@system.com / ${process.env.SUPERADMIN_PASSWORD || 'admin1234'}`)
